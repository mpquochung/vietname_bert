...
class CreateDataset:
  ...
  """
  def __init__ (self,sentences,labels1,model_name,batch_size=64,max_length=8):
    self.tokenizer=AutoTokenizer.from_pretrained(model_name, use_fast=False)
    self.batch_size=batch_size
    self.model_name=model_name
    self.sentences=np.array(sentences).tolist()
  ...
  def encoder_generator(self):
    ...
    tokenized_input = self.tokenizer(self.sentences, padding=True, truncation=True, max_length=self.max_length, return_tensors="pt") 
    self.input_ids = tokenized_input['input_ids'].to(DEVICE)
    self.attention_masks = tokenized_input['attention_mask'].to(DEVICE)
  ...
  def todataloader(self):
    ...
    self.dataset = TensorDataset(self.input_ids, self.attention_masks ,self.labels1)
